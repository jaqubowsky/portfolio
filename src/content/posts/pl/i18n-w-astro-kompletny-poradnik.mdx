...
Dla sitemapy skonfiguruj `@astrojs/sitemap` ze wsparciem i18n. Opcja `i18n` pluginu automatycznie dodaje linki hreflang dla stron o tej samej strukturze ścieżki (jak `/` i `/en/`). Ale strony i posty blogowe z **różnymi slugami per język** - `/blog/moja-droga-w-programowaniu/` vs `/en/blog/my-programming-journey/` - nie mogą być automatycznie dopasowane przez plugin. Zamiast ręcznie utrzymywać listę przetłumaczonych par URLi, używamy `buildHreflangMap()`, która generuje mapę bezpośrednio z `PATHNAMES` - jedynego źródła prawdy:
...
Każdy wpis w `PATHNAMES`, który ma ścieżki per locale, jest automatycznie uwzględniony w mapie hreflang sitemapy. Gdy dodasz nową przetłumaczoną stronę lub post, wystarczy zaktualizować `PATHNAMES` - sitemap przejmie to bez dodatkowej konfiguracji.
...
7. **`PATHNAMES`** to jedyne źródło prawdy dla zlokalizowanych URLi - nawigacja, hreflang i sitemap wyprowadzają dane z niego
