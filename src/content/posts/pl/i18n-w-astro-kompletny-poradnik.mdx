...
Każda decyzja związana z językiem w aplikacji odwołuje się do tego pliku. Dodanie trzeciego języka oznacza dodanie go tutaj i dostarczenie tłumaczeń — nic więcej się strukturalnie nie zmienia.
...
// Zbuduj zlokalizowaną ścieżkę URL
...
Przełącznik używa linków `hreflang` już obecnych w `<head>`, żeby znaleźć poprawny URL dla docelowego języka:
...
Kluczowy trik: dla zwykłych stron `stripLocaleFromPath` + `getLocalizedPathname` buduje poprawny URL. Ale dla postów blogowych z różnymi slugami w każdym języku (jak `/blog/moja-droga/` vs `/en/blog/my-journey/`), komponent odczytuje link `hreflang` z `<head>`, żeby uzyskać prawdziwy przetłumaczony URL.

## Krok 9: Detekcja języka przeglądarki

Automatycznie przekieruj nowych odwiedzających do ich preferowanego języka:
...
Ten skrypt uruchamia się raz na stronie głównej. Jeśli język przeglądarki pasuje do jednego z obsługiwanych języków (innego niż domyślny), przekierowuje i ustawia ciasteczko, żeby nie przekierowywać ponownie. Powracający odwiedzający lub ci, którzy użyli przełącznika języka, już mają ciasteczko.

## Krok 10: SEO — Hreflang i Sitemap

Twój layout powinien zawierać linki `hreflang` dla każdej wersji językowej:
...
Dla sitemapy skonfiguruj `@astrojs/sitemap` ze wsparciem i18n. Opcja `i18n` pluginu automatycznie dodaje linki hreflang dla stron o tej samej strukturze ścieżki (jak `/` i `/en/`). Ale posty blogowe mają **różne slugi w każdym języku** — `/blog/moja-droga-w-programowaniu/` vs `/en/blog/my-programming-journey/` — więc plugin nie jest w stanie samodzielnie ich dopasować. Musisz ręcznie zmapować te pary i wstrzyknąć linki hreflang przez hook `serialize`:
...
// Posty blogowe mają różne slugi w każdym języku, więc @astrojs/sitemap
// nie jest w stanie automatycznie wykryć, że są swoimi tłumaczeniami.
// Ręcznie mapujemy każdą parę [pl, en], żeby sitemap zawierała
// poprawne linki hreflang dla wyszukiwarek.
...
1. **Jeden plik konfiguracyjny** definiuje wszystkie obsługiwane języki — dodajesz nowy język w jednym miejscu
2. **Parametry rest `[...locale]`** obsługują routing — `undefined` dla domyślnego, kod języka dla pozostałych
3. **Pliki tłumaczeń** to zwykłe obiekty TypeScript z `as const` dla bezpieczeństwa typów
4. **Kolekcje treści** używają pola `lang` do filtrowania i `translationKey` do łączenia tłumaczeń
5. **Linki hreflang** w `<head>` służą podwójnie — SEO i nawigacja przełącznika języka
6. **Detekcja przeglądarki** uruchamia się raz i respektuje preferencję ciasteczka użytkownika

Żadna zewnętrzna biblioteka i18n nie jest potrzebna. Cały system to około 200 linii kodu narzędziowego, w pełni type-safe i działa z generowaniem statycznym Astro od razu.
