...
export function validateTranslations(posts: Post[]): Warning[] {
  ...
  for (const [key, localeMap] of byKey) {
    ...
    for (const [locale, post] of entries.slice(1)) {
      ...
      const refStructure = getStructureFingerprint(
        join(POSTS_DIR, `${refPost.id}.mdx`),
      );
      const postStructure = getStructureFingerprint(
        join(POSTS_DIR, `${post.id}.mdx`),
      );
      const refFp = refStructure.join(" → ");
      const postFp = postStructure.join(" → ");

      if (refFp !== postFp) {
        const refHeadings = refStructure.filter((s) =>
          s.startsWith("h"),
        ).length;
        const postHeadings = postStructure.filter((s) =>
          s.startsWith("h"),
        ).length;
        const refCode = refStructure.filter(
